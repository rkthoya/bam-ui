...
class EditModal extends React.Component {
  ...
  fetchMenus = (input, callback) => {
    axios
      .get(`/menus?search=${input}&per_page=5`)
      .then(({ data }) => {
  ...
  fetchMeals = (input, callback) => {
    axios
      .get(`/meals?search=name:${input}&per_page=5`)
      .then(({ data }) => {
  ...
  onEdit = () => {
    ...
    axios
      .put(`/menu-items/${this.props.menuItem.id}`, menuItem)
      .then(({ data }) => {
        this.props.onChange();
        this.setState({
          ...this.state,
  ...
  render() {
    ...
    const footer = (
      <Button color="primary" className="m-auto" onClick={this.onEdit}>
        Update Item
      </Button>
    );
    return (
